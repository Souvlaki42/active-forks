...
const appliesTests = [
  ...
  [{ if: [0, "apple", "banana"] }, null, "banana"],
  [{ if: [1, "apple", "banana"] }, null, "apple"],
  [{ if: [Math.PI, "apple", "banana"] }, null, "apple"],
  [{ if: [-1, "apple", "banana"] }, null, "apple"],
...
test("logging", () => {
  let last_console: number = 0;
  const logging = vi.spyOn(console, "log");
  logging.mockImplementation((logged: number) => {
    last_console = logged;
  });
...
test("Control structures don't eval depth-first", () => {
  ...
  jsonLogic.add_operation("push", (arg: boolean) => {
    i.push(arg);
    return arg;
  });
